...
// ACF templates
require_once('library/acf-templates/acf-bootstrap.php'); //advance custom fields templates
...
function bones_custom_admin_footer() {
	echo '<span id="footer-thankyou">Developed by <a href="http://www.abelltech.com/" target="_blank">Abelltech</a></span>. Built using <a href="http://www.abelltech.com/" target="_blank">Bones</a>.';
}
...
function theme_js(){
  ...
  wp_register_script('at-cycle2', get_template_directory_uri().'/library/js/cycle2.js');

  // wp_enqueue_script('less', array(''), '1.3.0', true);
  wp_enqueue_script('jquery');
  wp_enqueue_script('bootstrap', array('jQuery'), '1.1', true);
  wp_enqueue_script('at-cycle2', array('jQuery'), '1.1', true);
...
function getACFfield($fieldname,$parent='',$option='',$repeater=false,$notlink=true){
	if($repeater){
		if( get_field($parent,$option) ){
			while( has_sub_field($parent,$option) ){
				echo get_sub_field($fieldname);
			}
			
		}else{
			if($notlink){
				echo 'to add text here, go <a href="' . home_url( '/' ) . '"wp-admin/admin.php?page=acf-options">here</a>';
			}
		}
	}else{
